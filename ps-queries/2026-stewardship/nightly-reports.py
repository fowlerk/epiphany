...
# We accidentally included some test data in the final spreadsheet.
# So set the earliest date of actual data that we care about.
earliest_good_date = datetime(year=2025, month=10, day=4,
...
def reorder_rows_by_date(jotform_data):
    ...
    for row in jotform_data:
        ...
        if submit_date < earliest_good_date:
            continue
...
def statistics_graph(ps_members, ps_families, jotform, log):
    def _find_range(data, earliest, latest):
        for row in data:
            ...
            if submitted_dt < earliest_good_date:
                continue
...
def pledge_comparison_report(google, jotform_this_year, jotform_last_year, log):
    ...
    def _simplify_jotform(jotform_data,
    ...
    def _compare(this_year_data, log):
        ...
        for fduid in this_year_data:
            ...
            if previous_pledge == 0 or previous_pledge == 1:
    ...
    this_year = f'CY{stewardship_year-2000}'
    prev_year = f'CY{stewardship_year-1-2000}'

    filename = 'family-pledge-comparison-this-year-vs-last-year.csv'
    with open(filename, 'w') as fp:
        fields = ['Name', 'Family DUID', f'{this_year} pledge', f'{prev_year} pledge']
        writer = csv.DictWriter(fp, fieldnames=fields)
        writer.writeheader()
        ...
        for duid, data in this_year_data.items():
            item = {
                'Name' : data['name'],
                'Family DUID' : duid,
                f'{this_year} pledge' : data['current pledge'],
                f'{prev_year} pledge' : data['previous pledge'],
            }
            writer.writerow(item)
...
def convert_pledges_to_ps_import(ps_families, jotform, log):
    ...
    for pledge in jotform:
        ...
        # JMS ParishSoft doesn't store this data like PDS did.
        # We can load old pledge data from ParishSoft, but it's slow.
        # It is likely not worth showing here in this report.  So -- just comment it out.
        if False:
        ...
        row[f'CY{stewardship_year} frequency'] = pledge[f'CY{stewardship_year} how fullfill']

        # There was a bug in the Oct 2025 Jotform that didn't ask the "how"
        # question.  Hence the "how" field will always be blank.  :-(
        #row[f'CY{stewardship_year} how'] = pledge[f'CY{stewardship_year} how']
        ...
        # question.  Hence the "how" field will always be blank.  :-(
        #val = False
        #if 'Envelopes' in pledge[f'CY{stewardship_year} how']:
        #    val = True
...
def traverse_jotform_members(ps_families, ps_members,
    ...
    for jrow in jotform_list:
        ...
        if fduid not in ps_families:
            log.warning(f"WARNING: Family {fduid} submitted, but cannot be found -- skipping")
            continue
        ...
        for member_num in range(MAX_PS_FAMILY_MEMBER_NUM):
            ...
            if mduid not in ps_members:
                log.warning(f"WARNING: Member {mduid} submitted, but cannot be found -- skipping")
                continue
...
def member_ministry_csv_report(args, google, start, end, time_period,
    ...
    def _doit(output, filename_suffix, category_match, log):
        ...
        for ministry_name in sorted(output.keys()):
            ...
            for category in sorted(data.keys()):
                match = re.search(category_match, category)
                if match is None:
                    continue
    ...
    _doit(output, "interested", 'Interested', log)
    _doit(output, "needs human", 'Needs human', log)
...
def extract_member_extras(ps_families, ps_members, jotform_list, log):
    def _extract(jotform_row, member_jotform_num, member, state, log):
        item = dict()
...
def _export_gsheet_to_csv(service, start, end, google_sheet_id, fieldnames, log):
    ...
    # Second pass: rewrite the file with the desired column names
    with open(filename, newline='') as fp:
        ...
        for i, row in enumerate(csvreader):
            # Skip title row
            if i == 0:
                # This debug output is exceedingly helpful in determining
                # whether the constants.py fieldnames match the actual
                # Jotform-exported column names.
                col = 1
                log.debug("Column comparison: actual Jotform columns vs. python hard-coded columns from constants.py")
                for fieldname in fieldnames:
                    actual = row[fieldname]
                    desired = fieldname
                    log.debug(f"Column {col}: actual='{actual}' desired='{desired}'")
                    col += 1
            ...
            # Some Jotform submissions just have a blank FUID.
            # Shrug.  Skip them.
            if row['fduid'] == '':
                continue
...
def main():
    ...
    # JMS Ran the final one of these for Doug/Don
    if False:
    ...
    # JMS We did not ask these questions / they were not included in
    # 2026 eStewardship.  So there's no data to process.
    if False:
        # Per-ministry CSVs showing member status changes (given to
        # staff members to review, and ultimately to make phone calls
        # to followup).
